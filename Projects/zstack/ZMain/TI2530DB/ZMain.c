...
/*********************************************************************
 * @fn      dev_type_init
 * @brief   Initialize Device Type at start up.
 * @return  none
 *********************************************************************/
static void dev_type_init( void )
{
  uint8 logicalType;
  uint8 startOptions;
  uint8 size;
...
//  HalLedBlink ( HAL_LED_1, 50, 50, 500 );
//  HalLedBlink ( HAL_LED_2, 50, 50, 500 );
  
  size = ( uint8)osal_nv_item_len( ZCD_NV_LOGICAL_TYPE);
  osal_nv_read( ZCD_NV_LOGICAL_TYPE, 0, size, &logicalType);
#ifdef START_ENDDEVICE
  if( logicalType != ZG_DEVICETYPE_ENDDEVICE)
  {
    logicalType = ZG_DEVICETYPE_ENDDEVICE;
    osal_nv_write( ZCD_NV_LOGICAL_TYPE, 0, size, &logicalType);
    SystemReset();
  }
#elif START_ROUTER
  ...
  size = (uint8)osal_nv_item_len( ZCD_NV_STARTUP_OPTION);
  osal_nv_read( ZCD_NV_STARTUP_OPTION, 0, size, &startOptions);
  if( startOptions != ZCD_STARTOPT_AUTO_START)
  {
    startOptions = ZCD_STARTOPT_AUTO_START;
    osal_nv_write( ZCD_NV_STARTUP_OPTION, 0 ,size, &startOptions);
    SystemReset();
  }
}      // LJ
#endif
