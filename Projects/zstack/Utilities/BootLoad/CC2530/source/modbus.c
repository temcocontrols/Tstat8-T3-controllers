...
{
  if( buf_com[0] != MODBUS_SINGLE_READ && buf_com[1] != MODBUS_SINGLE_WRITE && buf_com[2] != MODBUS_MULTI_WRITE)
    return FALSE;
...
static void modbus_process_msg( uint8 *pData, uint8 len)
{
  if( pData[0] == modbus_id)
  {
    if(pData[1] == MODBUS_SINGLE_WRITE)
    {
    }
    else if( pData[1] == MODBUS_MULTI_WRITE)
    {
    }
    else if( pData[2] == MODBUS_SINGLE_READ)
    {
    }
  }
}